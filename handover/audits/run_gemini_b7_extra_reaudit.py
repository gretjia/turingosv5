#!/usr/bin/env python3
"""Gemini RE-AUDIT — Phase B B7-extra after dual-VETO fixes.
Independent of Codex. Output: GEMINI_B7_EXTRA_REAUDIT_2026-04-25.md"""
...
brief = """# Gemini RE-AUDIT — Phase B B7-extra (after VETO fixes)
...
**Mandate**: this is a RE-AUDIT. Your previous verdict (in `handover/audits/GEMINI_B7_EXTRA_PREBATCH_AUDIT_2026-04-25.md`) was **VETO** with high conviction; two specific VETO findings were:
- Q2.b: src/main.rs not in Trust Root manifest (call site of verify_trust_root)
- Q2.e: Cargo.lock not in Trust Root manifest (supply-chain dep-version vector)

Plus several CHALLENGE items. The user authorized FIX-THEN-PROCEED. The fixes have landed. Determine whether the original VETO is resolved AND whether the fix introduces new defects.

**Decision rule**:
- If both VETO items resolved + CHALLENGE items addressed (or have explicit deferral rationale) + no new P0 defect: **PASS** with HIGH conviction → batch authorized.
...
print(f"[gemini reaudit] prompt size: {len(full_prompt):,} chars", file=sys.stderr)
...
try:
    ...
    print(f"[gemini reaudit] HTTP {e.code}: {e.read().decode(errors='replace')[:2000]}", file=sys.stderr)
    sys.exit(1)

elapsed = time.time() - t0
print(f"[gemini reaudit] API returned in {elapsed:.1f}s", file=sys.stderr)

text = data["candidates"][0]["content"]["parts"][0]["text"]
out = ROOT / "handover/audits/GEMINI_B7_EXTRA_REAUDIT_2026-04-25.md"
header = (f"# Gemini PPUT-CCL Phase B B7-extra RE-AUDIT (post-VETO-fixes)\n"
          f"**Date**: 2026-04-25\n"
...
print(f"[gemini reaudit] saved: {out}")
