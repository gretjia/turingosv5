# Codex PPUT-CCL PREREG Adversarial Audit (Phase A4 ROUND 3)
**Date**: 2026-04-26
**Target**: handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md (round 3)
**Prompt size**: 550435 chars
...
session id: 019dc46b-464c-7fa3-9d54-a34730c8a3c1
--------
user
...
| WBCG_PPUT | sum over candidate artifacts Δ of `1[ArtifactState(Δ) = Certified]` where `Certified` requires the conjunction of all four heldout primary inferential tests rejecting null at family-corrected α (j-PPUT, j-FAR, j-RR, j-CPR per § 5.2) AND Rollbackable(Δ) = 1 (see § 1.7 for ArtifactState; § 7 Gate H for full reachability conditions) | Capability-compilation success |
...
Certified   : Δ entered Phase E AND survived all four primary heldout tests
              at family-corrected α (per § 5.2 + § 9): j-PPUT (ΔPPUT_heldout
              > 0), j-FAR (non-inferiority), j-RR (binomial vs p_0), j-CPR
              (non-inferiority). Counts toward WBCG_PPUT (§ 1.6 + § 7 Gate H).
Reverted    : Δ failed any of the four Phase E primary tests, OR caused
              regression in post-Phase-E audit, OR failed the rollback
              sub-eval (§ 6 E2). Permanently flagged; rollback executed;
              cannot re-enter promotion within this arc (resubmission requires
              new artifact_id + design change per C-035).
```
...
1. `tests/heldout_operational_sealing.rs` (Phase B7 implementation; **5 layers**):
   ...
   **L3 — Tool call pre-flight filter (existing, expanded round 3)**:
   - File-reading tool calls (`cat` / `Read` / `head` / etc.) issued by Phase B-D agents pre-flight-checked: path matches blacklist → `EPERM`.
   - **Round 3 addition**: tool call ARGUMENTS scanned for the literal seed string; match → `EPERM`. This blocks `echo "20260426_PPUT_CCL" | sha256sum`-style recomputation attempts.

   **L4 — Hashlib gate (round 3 new)**:
   - Conformance: any agent code path that imports `hashlib` (Python) or `sha2` / `Sha256` (Rust) AND has read access to a file containing the seed string is BLOCKED at static analysis time (grep both ways, intersect).
   - For dynamic invocation: agents issuing a tool call that runs hashlib with the seed string as input → `EPERM` (the tool call argument scanner from L3 catches this).
   - Whitelist: `handover/preregistration/scripts/split_pput_ccl.py` is the only authorized hashlib-with-seed code path; it runs ONCE at Phase A2 and never again.

   **L5 — MiniF2F/Test problem ID list blacklist (round 3 new)**:
   - The 244-problem MiniF2F/Test ID list is itself blacklisted from Phase B-D agent context: agents cannot enumerate the source pool from which heldout was drawn.
   ...
   - This means even given the seed + bucket rule, the agent has no `pid` set to bucket — they would need MiniF2F/Test enumeration which is also blocked.
   - Whitelist: `bin/heldout_evaluator.rs` reads the full list at Phase E.
...
## 5. directional hypotheses (revised round 2 per dual audit M1)

Each H1-H4 has **exactly one primary inferential endpoint** at α level (others = descriptive). H5 is reclassified as a **deterministic gate**, not an inferential test. Per-artifact heldout guardrails (ΔFAR/RR/ΔCPR/ΔPPUT) enter the family at Phase E with k = number of artifacts (see § 9 for full family construction).

### 5.1 Primary inferential hypotheses (Phase C ablation, family member 1-4)

| ID | Hypothesis | Phase | Primary endpoint (one test, one-sided) | Independent unit | Direction | Descriptive secondary endpoints (NOT in family) |
|---|---|---|---|---|---|---|
...
| H2 | Panopticon pollutes context — its H-VPPUT is lower than Full | C | Paired sign test on per-(problem, seed) `VPPUT` values, Full vs Panopticon | (problem, seed) | Full > Panopticon | CPR per mode; IAC per mode; mean prompt length per mode |
| H3 | Amnesia loses recovery — its H-VPPUT is lower than Full | C | Paired sign test on per-(problem, seed) `VPPUT` values, Full vs Amnesia | (problem, seed) | Full > Amnesia | ERR per mode; rollback count per mode |
| H4 | Homogeneous (skill-uniform swarm) has lower H-VPPUT than Full | C | Paired sign test on per-(problem, seed) `VPPUT` values, Full vs Homogeneous | (problem, seed) | Full > Homogeneous | IAC per mode; pairwise payload Jaccard |

**Independent unit clarification**: per-(problem, seed) tuple. With hard-10 adaptation × 2 random Boltzmann routing seeds = **20 paired (problem, seed) observations per condition pair**. McNemar uses paired-discordant counts on these 20 pairs.

### 5.2 Phase E heldout family members (5 → 5+4k)

For each artifact j ∈ {1, ..., k} that enters Phase E with `ArtifactState = Quarantined`, four primary inferential tests run on heldout-54:

| ID family | Hypothesis (one per artifact j) | Primary endpoint | Independent unit | Direction |
|---|---|---|---|---|
| `j-PPUT` | Artifact j increases held-out VPPUT | One-sided binomial test on per-(problem, seed) sign of `VPPUT_with_j − VPPUT_minus_j` (leave-one-out within sealed eval; see § 6 E3) | (problem, seed) | sign > 0 |
| `j-FAR` | Artifact j does not increase FAR | One-sided non-inferiority binomial test on per-(problem) sign of `FAR_with_j − FAR_minus_j` | problem | non-positive |
| `j-RR` | Artifact j regression rate ≤ baseline `p_0` | One-sided exact binomial on count of regressions (heldout problems SOLVED at baseline UNSOLVED with j) vs pre-registered baseline rate `p_0` measured on adaptation pre-Phase-D | problem | RR_j ≤ p_0 |
| `j-CPR` | Artifact j does not increase CPR | One-sided non-inferiority binomial on per-(problem) sign of `CPR_with_j − CPR_minus_j` | problem | non-positive |

`p_0` (baseline regression rate) — full calibration protocol (round 3 closure of Codex R3-2 tuning-surface concern):
...
Ceiling:       if measured p_0 > 0.10, it indicates the simulated-rollback toggle
                 is too aggressive (or the system is too brittle); ABORT,
                 redesign toggle, redo Phase B7. p_0 > 0.10 is a BLOCKER, not
                 a "high baseline that auto-passes future tests".
Dual audit:    p_0 calibration result included in the audit packet for the
                 Phase B → Phase C transition checkpoint. Codex + Gemini
                 sanity-check that the value is plausible and the protocol was
                 followed.
```

This protocol forecloses the round-2 tuning-surface attack: every parameter (toggle, sample, estimator, schedule, freeze point, ceiling) is pre-committed; the calibration runs are observable jsonl; the value enters Trust Root. There is no analyst degree of freedom left except the 0.10 ceiling, and the ceiling is itself an abort condition (high p_0 ⇒ design problem, not an opportunity to pass weaker artifacts).

### 5.3 Family total

```
family_size = 4 (Phase C) + 4 × k (Phase E, k = artifacts entering heldout)
```

k is unknown until Phase D ends. Multiplicity correction (§ 9) handles dynamic family size via Holm-Bonferroni stepwise procedure declared a priori.

### 5.4 H5 — deterministic gate (NOT in inferential family)

```
Gate H: count of artifacts j with ArtifactState(j) = Certified ≥ 1
```

`Certified` requires j-PPUT, j-FAR, j-RR, j-CPR ALL to reject the null at the family-corrected α level (per § 5.2 + § 9). It is the conjunction of inferential outcomes, not a separate inferential test. Reaching Gate H = 1+ artifacts pass all four heldout tests after correction. H5 is therefore a deterministic OR over the artifact-level outcomes; no separate p-value.
...
- E3 Statistical inference per § 5.2 + § 9. For each j, evaluate the four primary tests (j-PPUT, j-FAR, j-RR, j-CPR). Holm-Bonferroni over the full family `4 + 4k`.
...
| H | **Heldout WBCG_PPUT > 0 with ArtifactState = Certified gate** (round 2 — RR criterion relaxed per Gemini audit M4): at least one ArchitectAI-generated user-space artifact Δ has `ArtifactState(Δ) = Certified` per § 1.7 — meaning (i) was Accepted (passed § 3 + § 3.5 AuditorAI battery), (ii) was Quarantined (user-approved + ΔPPUT_meta_val > 0 + N_use ≥ 3 on meta_val), (iii) at heldout sealed eval all FOUR primary inferential tests reject the null at family-corrected α (per § 5.2 + § 9): **j-PPUT** (ΔPPUT_heldout > 0 by paired binomial), **j-FAR** (ΔFAR ≤ 0 by non-inferiority), **j-RR** (per-problem regression rate ≤ baseline `p_0` by exact one-sided binomial — NOT the brittle `RR = 0` from round 1), **j-CPR** (ΔCPR ≤ 0 by non-inferiority), AND (iv) is rollbackable (artifact directory deletion at the protocol level restores prior heldout PPUT — verified by an explicit rollback sub-eval, see § 6 E2). Quarantined-only artifacts (failed promotion) do NOT count toward Gate H. |
...
## 9. multiplicity (revised round 3 per Codex round-2 audit)

### 9.1 Family construction with hard-cap k_max (round 3 closure of family-timing defect)

**Round 2 had a timing flaw**: Gate C decisions occur in Phase C (before Phase D ends, before k is known); but Holm thresholds depended on the final N = 4 + 4k. Round 3 closes this:

```
k_max = 10  (a priori upper bound on artifacts entering Phase E)
N_max = 4 + 4 × k_max = 44  (frozen family size for ALL inferential decisions)
```

`k_max = 10` is **pre-committed** at Phase A5 commit gate. Realistic Phase D throughput estimates (D5 SLA + 7-day window) bound expected k ≤ 8; k_max = 10 gives 25% headroom. **If realized k > k_max**: arc aborts at end of Phase D; user may submit a formal addendum (with new audit) raising k_max, but ALL inferential decisions in the original PREREG remain at N_max = 44. No re-running of Phase C with relaxed thresholds.

If realized k < k_max: we are conservative (lose power) — accepted tradeoff for prereg discipline. Spare family slots (positions k+1 .. k_max in Holm sort order) are simply not used; their α budget is forfeit.

**Gate C decisions** (H1-H4 in Phase C): Holm thresholds use N_max = 44 throughout. Even though k is unknown at Gate C time, the family is frozen at 44 from PREREG commit forward.

**Phase E decisions** (j-PPUT, j-FAR, j-RR, j-CPR for each Quarantined artifact j): same family of N_max = 44. The 4 Phase C tests + 4·k actual Phase E tests sort within the same Holm procedure.

H5 (Gate H = at least one `Certified` artifact) is **NOT** a separate inferential test — it is a deterministic OR over per-artifact outcomes that already passed the family-corrected α (per § 5.4).

### 9.2 Independent unit = per-problem with seeds as replicates (round 3 fix)

**Round 2 was wrong**: declared independent unit = (problem, seed) tuple, giving 20 paired observations for hard-10 × 2 seeds. But seeds within the SAME problem are correlated (same theorem, same Mathlib state, same scaffold) — they are noise-reducing replicates, not independent draws.

**Round 3 fix**: independent unit = **per-problem**. For each problem p, both seeds run; per-problem score is the **paired-binary outcome on aggregated per-problem PPUT**:
- For each (mode_pair, problem): compute mean VPPUT under each mode across the 2 seeds.
- Paired sign: 1 if mode_full mean > mode_alt mean for problem p, 0 otherwise.
- McNemar uses 10 paired-binary observations (one per problem in hard-10).

This gives a more honest sample size (n = 10) at the cost of test power — but it is statistically correct under the within-problem-correlation reality.

### 9.3 Correction procedure: Holm-Bonferroni stepwise (a priori) — applied to N_max = 44

Family-wise α = 0.05. For all 4 + 4·k_max = 44 hypothesis slots:

1. Compute p-values for the 4 Phase C tests (always populated) + 4·k Phase E tests (k determined at Phase D end; remaining 4·(k_max−k) slots = "not run", treated as accepting null).
2. Sort all populated p-values ascending: `p_(1) ≤ p_(2) ≤ ... ≤ p_(M)` where M = 4 + 4k.
3. For i = 1, 2, ..., M:
   - Compare `p_(i)` to threshold `α_family / (N_max - i + 1) = 0.05 / (44 - i + 1)`.
   - If `p_(i) > threshold`: STOP. Reject hypotheses 1..i-1, accept null for hypotheses i..M.
   - If `p_(i) ≤ threshold`: continue to i+1.

The denominator uses N_max = 44, not the realized M. This is conservative (since unused slots cannot make any decision more lenient).

### 9.4 Worked example (round 3)

If k = 3 (3 artifacts enter Phase E): M = 16 populated tests, but Holm uses N_max = 44.
- p_(1) threshold = 0.05/44 = 0.00114
- p_(2) threshold = 0.05/43 = 0.00116
- ...
- p_(16) threshold = 0.05/29 = 0.00172

If k = 10 (max realistic): M = N_max = 44.
- p_(1) threshold = 0.05/44 = 0.00114
- p_(44) threshold = 0.05/1 = 0.05

If k = 0 (no artifacts entering Phase E, only Phase C tests):
- M = 4; p_(1) = 0.05/44 = 0.00114; p_(4) = 0.05/41 = 0.00122

### 9.5 Power expectation (round 3 — recomputed for per-problem unit)

For McNemar paired one-sided at α = 0.00114 (smallest Holm threshold) on **n = 10 per-problem paired-binary observations**:
- Effect required for rejection: `b ≥ 8` paired discordants in predicted direction (vs `c ≤ 2` opposite). This is **identical to Paper 1 v2 effect** (B vs A: 8 hard problems uniquely solved by B, 0 by A).
- Phase C ablations are powered for effect sizes consistent with prior architect-predicted strengths. Smaller effects → underpowered → declare NEGATIVE finding rather than enlarge N (per feedback_phased_checkpoint).

For Phase E per-artifact j-PPUT test on n_problems = 54 (heldout), one-sided sign test at α = 0.05/29 (mid-Holm threshold for k=10):
- Detectable effect: `b ≥ 36` of 54 problems where artifact j improves PPUT (vs `c ≤ 18` opposite); strong but realistic for a genuinely useful artifact.
- For weaker artifacts (b around 32-35), the test is borderline; PREREG accepts NEGATIVE-finding outcome at this scale.

### 9.6 Pre-registration constraint

Family construction (which tests, which directions, which thresholds, k_max = 10, N_max = 44) is FROZEN at Phase A5 commit gate. No tests added post-hoc. No threshold relaxation post-hoc. No k_max increase without formal addendum + dual external re-audit.
...
|---|---|
| Phase C all H1-H4 fail | "Three constitutional ablations did not significantly affect VPPUT on hard-10 adaptation × N=20 within Phase C statistical power; either ablations were under-effective or VPPUT signal too noisy at this scale" |
...
| Phase D ≥ 1 Quarantined, Phase E zero Certified | "Phase D produced k Quarantined artifact(s) with positive ΔPPUT on meta_validation; on heldout-54 sealed eval, no artifact survived all four primary inferential tests (j-PPUT / j-FAR / j-RR / j-CPR) at family-corrected α; capability-compilation gain on Phase D set did not generalize to heldout under Holm-Bonferroni correction" |
...
# Codex PPUT-CCL PREREG Adversarial Audit (Phase A4 ROUND 2)
**Date**: 2026-04-26
**Target**: handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md (round 2)
**Prompt size**: 279775 chars
...
reasoning effort: xhigh
reasoning summaries: none
session id: 019dc464-28f9-7e11-b2f5-b004a59e4055
--------
user
...
Real sealing in this arc is **operational**: enforced by access pattern + conformance testing. We rely on:

1. `tests/heldout_operational_sealing.rs` (Phase B7 implementation):
   - **Read isolation in code paths**: greps `experiments/minif2f_v4/src/`, `src/`, `bin/`, plus any agent loader, for any read of `heldout_*.txt`, the splits JSON's heldout key, or substrings matching the heldout list. Only `bin/heldout_evaluator.rs` is whitelisted (Phase E gate runner).
   - **Read isolation in agent context**: explicit gate at agent prompt construction — `PREREG_PPUT_CCL_2026-04-26.md` and `PPUT_CCL_SPLITS_2026-04-26.json` are on a context-blacklist; any agent prompt that includes their bytes triggers `EHELDOUT_CTX_LEAK` abort.
   - **Read isolation in tool calls**: file-reading tool calls (e.g., `cat` / `Read`) issued by Phase B-D agents have a pre-flight filter: if path matches the blacklist, return `EPERM`.
   - **Re-bucketing prohibition in agents**: agents cannot import `hashlib` + the seed string and recompute buckets. Conformance: greps for the literal seed string `20260426_PPUT_CCL` in any code path readable by Phase B-D agents.
2. CI / `cargo test` MUST run this conformance battery pre-commit.
3. Heldout problems NEVER appear in adaptation / meta_val runs. Detected leak = BLOCKER, arc invalidated, must respin with a new seed string AND a new heldout draw.

**Residual risk** (acknowledged honestly): if a future ArchitectAI is creative enough to construct an artifact that — by being a function of the seed string + bucketing rule, both of which are necessary public information for reproducibility — implicitly encodes the heldout list, this defense fails. This is why M6 (§ 3.5 lookup-table evasion predicates, round 2) tightens the artifact content checks to disallow problem-ID enumeration in artifact bodies.
...
### Phase C — Ablation smoke tests (days 11-17)
- C1 **5 modes implemented as a single `--mode` CLI flag on one binary** (Patch E from Gemini DeepThink): `evaluator --mode {full, panopticon, amnesia, soft_law, homogeneous}`. Modes change runtime behavior; binary bytes do not change between modes.
- C2 **hard-10 adaptation × 2 random Boltzmann routing seeds = 20 paired (problem, seed) observations per condition pair** (round 2: independent unit clarified per dual audit M1). Hard-10 sample is drawn from the adaptation set (144 problems) using the same hash-based deterministic procedure as the three-split: `random.Random("hard10_pput_ccl_seed").sample(adaptation_set, 10)`. The 10 problem IDs + their fingerprint are pre-committed in Phase A5. Boltzmann seeds = `[31415, 2718]`. Total per-mode runs: 5 modes × 10 problems × 2 seeds = 100 jsonl rows. McNemar per condition pair uses the 20 paired discordant counts.
- C3 verify H1-H4 (§ 5) on PPUT axis
- C4 CHECKPOINT_PHASE_C document + 7-redline check + dual external audit
- C5 **Mode-purity conformance**: `tests/mode_flag_binary_purity.rs` asserts that running all 5 modes on the same problem produces jsonl with **identical `git_sha`, `binary_sha256`, and `model_snapshot` fields** — only the `mode` field differs. Any drift = BLOCKER (rules out "Soft Law happened to use a different binary" confound).
- **Gate C**: H1-H4 each pass at Bonferroni α (§ 9); IF any H1-H4 fails, diagnose root cause, do NOT enlarge N (per feedback_phased_checkpoint). Heldout never touched.
...
## 9. multiplicity (revised round 2 per dual audit M1)

### 9.1 Family construction

Family-wise inferential tests = `4 (Phase C) + 4 × k (Phase E)` where k = count of artifacts entering Phase E with `ArtifactState = Quarantined`. k is unknown until end of Phase D; the multiplicity procedure handles dynamic k through Holm-Bonferroni stepwise rejection (declared a priori).
...
For all `4 + 4k` p-values:

1. Sort ascending: `p_(1) ≤ p_(2) ≤ ... ≤ p_(N)` where `N = 4 + 4k`.
2. For i = 1, 2, ..., N:
   - Compare `p_(i)` to threshold `α_family / (N - i + 1) = 0.05 / (N - i + 1)`.
   - If `p_(i) > threshold`: STOP. Reject hypotheses 1..i-1, accept null for hypotheses i..N.
   - If `p_(i) ≤ threshold`: continue to i+1.

Holm-Bonferroni controls FWER at 0.05 (same as vanilla Bonferroni at α/N) but is uniformly more powerful — earlier hypotheses get less stringent thresholds.

### 9.3 Worked example

If k = 3 (3 artifacts enter Phase E):
- N = 4 + 4×3 = 16
- p_(1) threshold = 0.05/16 = 0.00313
- p_(2) threshold = 0.05/15 = 0.00333
- ...
- p_(16) threshold = 0.05/1 = 0.05

If k = 10 (max realistic):
- N = 44
- p_(1) threshold = 0.05/44 = 0.00114
- p_(44) threshold = 0.05

### 9.4 Power expectation for hard-10 × N=20 paired

For McNemar paired one-sided at α = 0.00313 (k=3 case, smallest threshold) on 20 paired observations:
- Detectable effect ≈ b ≥ 8 paired discordants in the predicted direction (vs. baseline c ≤ 2 in opposite direction).
- This is consistent with prior Paper 1 v2 effects (B vs A: 8/8 discordants) — adequate for Phase C ablations to register if architect-predicted effect sizes hold.
- If effect sizes are smaller, Phase C may be underpowered. PREREG accepts this tradeoff explicitly: rather than enlarge N at risk of confound, declare a NEGATIVE finding (Soft Law / Panopticon / Amnesia not detected at hard-10 N=20) and document the power assumption in the write-up.

### 9.5 Pre-registration constraint

Family construction (which tests, which directions, which thresholds) is FROZEN at Phase A5 commit gate. k itself is data-driven (Phase D output); but the formula `N = 4 + 4k` is pre-committed. No tests added post-hoc.
...
codex
I’ll treat the round-2 PREREG pasted in the prompt as the audit corpus. I’m not reading any Gemini audit files; I’m only evaluating the stated fixes against the round-1 blockers. 

codex
I did not read any `GEMINI_PPUT_CCL_AUDIT_ROUND2_*.md`. This round-2 audit is based only on the PREREG text pasted here.

**Round-2 P0 Closure**

| Finding | Status | Reason |
|---|---|---|
| **P0-leak** | **partial** | §2.3 now honestly frames SHA-256 as integrity, not confidentiality, but the operational seal is incomplete: if Phase B-D agents can access the seed through the split script, audit artifacts, prompt text, or dynamic tool args, they can recompute buckets without reading blacklisted PREREG/SPLITS; the “grep literal seed in readable code paths” rule does not fully cover runtime tool calls or all seed-bearing files. |
| **P0-measure** | **closed, with claim caveat** | §6 E2 cleanly defines BASELINE + ALL + k LOO sub-evals on the same heldout-54 × 3 seeds; compute check is correct: `54×3×12×50s = 97,200s ≈ 27h` at `k=10`; single-touch holds if results are computed once. LOO handles interactions only as conditional marginal contribution given the rest of Q, so later claims must not imply standalone artifact efficacy. |
| **P0-fam** | **still-open** | §5/§9 close the obvious family-count omission, but §5.1/§5.2 still treat `(problem, seed)` as independent despite within-problem seed correlation, and §6 Gate C requires H1-H4 to pass before k is known even though §9 Holm thresholds depend on final `N=4+4k`. |
| **P0-stat-gemini** | **partial** | §5.2 now includes per-artifact j-PPUT/j-FAR/j-RR/j-CPR in the family, but the same independence and dynamic-k/Gate-C timing defects mean family closure is not yet statistically clean. |
| **P0-gate-h** | **partial** | §7 replaces brittle `RR=0` with an exact binomial test against `p_0`, improving reachability, but §5.2 defines `p_0` as “ablating any single recent code change” without fixing the code-change set, estimator, seeds, sample, or freeze point before artifact generation; this can be tuned high enough to auto-pass. |
| **P0-spof** | **closed** | §6 D5 now gives a concrete 48h SLA, deferred queue, no clock pause, and ≥5 queued >48h early-abort rule; partial availability degrades throughput but no longer blocks the arc indefinitely. |

**VERDICT: CHALLENGE**

Top 3 still-open/new P0s:

1. **STAT family still not executable as preregistered**: §6 Gate C occurs before k is known, while §9 Holm correction depends on final `4+4k`; plus `(problem, seed)` is not a defensible independent unit without clustering or per-problem aggregation.

2. **RR baseline `p_0` is a new tuning surface**: §5.2 must specify the exact Phase B calibration protocol and freeze `p_0` before Phase D artifact generation, with an immutable audit trail.

3. **Heldout operational sealing remains leaky under recomputation**: §2.3 must explicitly deny agents access to every seed-bearing artifact, including the split script and audit/prereg echoes, and block dynamic hash recomputation tool calls, not just file reads of PREREG/SPLITS.

Conviction: **high**. The revision fixes several round-1 textual contradictions, but the remaining issues are preregistration-level defects, not implementation details.
tokens used
92,047
I did not read any `GEMINI_PPUT_CCL_AUDIT_ROUND2_*.md`. This round-2 audit is based only on the PREREG text pasted here.
...
codex
I’ll audit the round-3 text as pasted and keep the focus narrow: the three round-2 Codex blockers plus round-3 side effects. I’m treating internal consistency as part of closure, because a preregistration has to be executable without choosing between conflicting sections.
codex
I did not inspect any Gemini round-3 artifact. This is based only on the pasted round-3 PREREG.

**Round-2 P0 Closure**

| P0 | Status | Reason |
|---|---|---|
| **P0-fam** | **still-open** | §9.1’s `N_max=44` closes the Gate-C timing flaw, but the independent unit is internally inconsistent: §5.1 still says `(problem, seed)` / 20 observations while §6 C2 and §9.2 say per-problem / n=10; §5.2 also still lists j-PPUT as `(problem, seed)` while §9.5 analyzes heldout as n=54 problems. |
| **P0-gate-h** | **partial** | §5.2 now fully specifies and freezes `p_0`, closing the tuning-surface issue, but the new `p_0 ≤ 0.10` exact-binomial criterion appears mathematically unreachable for j-RR under §7 + §9 Holm thresholds: even 0/54 regressions gives best-case p `0.9^54 ≈ 0.0034`, above early `N_max=44` thresholds. |
| **P0-leak** | **partial** | §2.3 L2/L5 substantially improves sealing, but L3/L4 only explicitly catch literal seed-string arguments and selected hash libraries; concatenated seed construction, alternate SHA-256 call sites, and source-pool enumeration via shell tools need explicit blocking to fully close recomputation. |

**VERDICT: CHALLENGE**

Top 3 blockers:

1. **Statistical unit/power inconsistency remains**: §5.1, §6 C2, §9.2, and §9.5 disagree on whether the unit is `(problem, seed)` or `problem`. Also §9.5’s Phase C power claim is wrong: with n=10 at α≈0.00114, 8/10 is not significant; a sign test needs 10/10.

2. **j-RR certification is likely impossible as written**: §5.2 caps `p_0` at 0.10, while §7 requires j-RR to reject at family-corrected α. With heldout n=54, zero regressions still cannot clear early Holm thresholds under `N_max=44`. If RR is meant as a guardrail non-rejection instead, §1.6/§7 must stop saying all four tests reject.

3. **Hash recomputation defense is still too literal**: §2.3 L3/L4 should block all seed-derived construction and all hash-function routes used by Phase B-D agents, plus explicit `ls/find/rg` enumeration of MiniF2F/Test, not just literal `20260426_PPUT_CCL` and `hashlib`/`sha2`.

`k_max=10` itself is acceptable as a conservative precommitment because §9.1 aborts rather than relaxes thresholds. §10 R3-4 is strict enough: it correctly limits artifact claims to marginal contribution conditional on `Q \ {j}`.

Conviction: high. I am concerned by three revision rounds only in a narrow sense: not structural failure of the thesis, but the prereg statistical sections are still being patched in conflicting places rather than cleanly rewritten end-to-end.
tokens used
173,983
I did not inspect any Gemini round-3 artifact. This is based only on the pasted round-3 PREREG.
