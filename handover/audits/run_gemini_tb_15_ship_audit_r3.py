...
ENV_FILES = [ROOT / ".env"]
ROUND = os.environ.get("TB15_AUDIT_ROUND", "R3")
...
# ── Brief ──
brief = """# Gemini TB-15 R3 Ship Audit — POST R2-VETO-CLOSURE

## R3 CONTEXT — Codex R2 VETO closure (READ THIS FIRST)

**R2 verdicts (2026-05-04)**:
- **Gemini R2: PASS** (you cleared all R1 findings including your own Q12 VETO; only minor OBS recommendations: compile-const upgrade + parser test hardening). Conviction high.
- **Codex R2: VETO** conviction high — Q3 + TB15-CAS-ID NEW findings:
  - `LATEST_MARKOV_CAPSULE.txt` published `a94ae884...` but CAS index keyed Markov object under `e4932fca...` (different sha256). `cas.get(&capsule.capsule_id)` would FAIL.
  - Root cause: writer computed `capsule_id = sha256(prelim_bytes)` then stored DIFFERENT `final_bytes` (with capsule_id+sha256 populated) whose sha256 differs.
  - Same bug in `write_autopsy_capsule` and `derive_autopsies_for_bankruptcy`.
  - Breaks SG-15.3 next-session bootstrap and Markov chain resolvability.

**R3 remediations applied** (verify each architecturally):

1. **Q3 + TB15-CAS-ID closure**: writers rewritten so `capsule_id == sha256(stored_bytes)`. Specifically: build capsule with capsule_id=Cid::default()/sha256=Hash::ZERO; canonical_encode → THOSE are the bytes stored in CAS; capsule_id = Cid::from_content(stored_bytes); populate the in-memory struct fields AFTER CAS write (caller gets ergonomic populated view; on-CAS bytes remain the zeroed-identity form).
2. **NEW helpers**: `restore_markov_capsule_from_cas_bytes(bytes)` and `restore_autopsy_capsule_from_cas_bytes(bytes)` canonical-decode + re-derive capsule_id/sha256 from `Cid::from_content(&bytes)`. Symmetric round-trip.
3. **NEW `BankruptcyAutopsyDerivation` struct**: replaces the old `(AgentAutopsyCapsule, Vec<u8>)` tuple in `derive_autopsies_for_bankruptcy`'s return type. Carries `capsule + private_bytes + stored_capsule_bytes`. Apply_one writes `stored_capsule_bytes` (the EXACT bytes whose sha256 == capsule_id).
4. **NEW round-trip tests**: `write_markov_capsule_cas_resolvable_by_capsule_id` + `write_bankruptcy_autopsies_to_cas_round_trip` (extended) — both assert `cas.get(&cap.capsule_id)` succeeds + retrieved bytes' sha256 == capsule_id + restore round-trip works.
...
- Trust Root: GREEN (sequencer.rs + fc_alignment_conformance.rs rehashed)
- R3 evidence at `handover/evidence/tb_15_markov_capsule_2026-05-04/` includes `cas_index.jsonl`

## R3 mandate

Verify the Codex R2 VETO is now closed. As the architectural strategic reviewer, focus on:

1. **Architectural soundness of the writer pattern**: is "store the zeroed-identity bytes; compute capsule_id from them; populate in-memory struct after" the right pattern? Are there cleaner alternatives (e.g., serde-skip the capsule_id field)? Is the asymmetry between on-CAS bytes (zeroed) and in-memory struct (populated) a footgun for future consumers?

2. **Restore helpers correctness**: do `restore_markov_capsule_from_cas_bytes` and `restore_autopsy_capsule_from_cas_bytes` correctly reverse the write operation? What happens if a consumer canonical-decodes the CAS bytes WITHOUT calling restore? They'd get a struct with capsule_id=Cid::default() — is that an invariant violation that would propagate silently?

3. **BankruptcyAutopsyDerivation struct API**: is the new return type ergonomic? Does it correctly carry the bytes to apply_one without recomputation? Is it idempotent?

4. **Cross-cut to TB-11**: TB-11 `write_evidence_capsule` likely has the same bug. Should TB-15 R3 ship include a TB-11 fix, or carry-forward as OBS?

5. **NEW findings welcome** but per `feedback_audit_loop_roi_flip`: if R3 challenges shift to test-scaffold edges, recommend ship-with-OBS rather than R4.

Emit a fresh verdict (PASS / CHALLENGE / VETO).

---

## R2 CONTEXT (for reference)
