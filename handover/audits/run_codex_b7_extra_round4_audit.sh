#!/usr/bin/env bash
# Codex ROUND-4 RE-AUDIT — Phase B B7-extra after dual-VETO fixes (commit 15b87fb +
# code-simplifier 438a648 + constitution amendment c061450 + audit packet df77b0a).
#
# Independent of Gemini. Output: CODEX_B7_EXTRA_ROUND4_AUDIT_2026-04-25.md
set -euo pipefail

ROOT="/home/zephryj/projects/turingosv4"
OUT="${ROOT}/handover/audits/CODEX_B7_EXTRA_ROUND4_AUDIT_2026-04-25.md"
TMP_PROMPT="$(mktemp /tmp/b7_extra_codex_round4_audit.XXXXXX.md)"
...
echo "[codex round4_audit] prompt size: ${prompt_size} chars" >&2

t0=$(date +%s)
{
  printf '# Codex PPUT-CCL Phase B B7-extra ROUND-4 RE-AUDIT (post-VETO-fixes)\n'
...
t1=$(date +%s)
elapsed=$((t1 - t0))
echo "[codex round4_audit] API returned in ${elapsed}s" >&2
echo "[codex round4_audit] saved: $OUT" >&2
