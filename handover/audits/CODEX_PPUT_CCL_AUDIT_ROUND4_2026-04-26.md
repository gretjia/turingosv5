# Codex PPUT-CCL PREREG Adversarial Audit (Phase A4 ROUND 4)
**Date**: 2026-04-26
**Target**: handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md (round 4)
**Prompt size**: 336144 chars
...
session id: 019dc478-d96c-7a30-a9ab-27b6f6c0db05
--------
user
...
codex
I’m checking the round-4 text against the three blockers from my round-3 verdict, then doing a consistency pass for stale non-changelog language. The main thing I’m watching is whether any remaining issue changes the preregistered decision rule, not just wording.
exec
/bin/bash -lc "python3 - <<'PY'
import math
for b in range(35,41):
    p=sum(math.comb(54,x) for x in range(b,55))/2**54
    print(b, p)
print('alpha', 0.05/34)
# RR impossible check 0.9"'^54 and threshold 0.05/44/34
print('"'0.9"'^54'"', 0.9**54, '0.05/44', 0.05/44, '0.05/34', 0.05/34)
PY" in /home/zephryj/projects/turingosv4
 succeeded in 0ms:
35 0.02011178204785835
36 0.009917163364034143
37 0.0045366701697935885
38 0.0019191329401630486
39 0.0007481294426967544
40 0.00026771775142853116
alpha 0.0014705882352941176
0.9^54 0.0033813919135227306 0.05/44 0.0011363636363636365 0.05/34 0.0014705882352941176

exec
/bin/bash -lc "rg -n \"\\(problem, seed\\)|per-\\(problem, seed\\)|all four|four primary|4 \\+ 4|4\\+4|N_max = 44|N=20|heldout-49|family=5|family_size = 4 \\+ 4|j-RR.*inferential|RR = 0\" handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md 2>/dev/null" in /home/zephryj/projects/turingosv4
 succeeded in 0ms:
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:36:   - hard-10 adaptation × N=20 paired
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:159:- 同一 N=20 sample 同一天早些的 Phase 8 reasoner baseline: 8/20 solves（reasoner）
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:360:- Branch `feat/tape-economy-v1` (worktree), N=20 sample
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:451:  - McNemar stat 4.0 → one-sided exact p ≈ 0.0625 (N=20 borderline); effect is unambiguously positive
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:474:### F-2026-04-19-01: TEMP_LADDER mechanism validated on N=20 sample
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:481:- McNemar (b=3,c=0) one-sided p≈0.125 on N=20 — needs N=50 for stat-sig
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:489:- Cheapest mechanism-level intervention; testable in <1h on N=20 sample
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:575:- N=20 full A/B launched (v40_bundle_v5, timestamp 20260416T...)
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:582:- 104 tests pass. Running N=20 A/B (timestamp 20260416T195805)
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:743:3. Quick A/B on **N=20 subset** (sample_N20_S74677.txt, fp=8d390ee4eef82dbb)
handover/ai-direct/AUTO_RESEARCH_NOTEPAD.md:750:**Power analysis**: N=20 detects Δ=3 with 57% power; Δ=5 with >80%. Same as N=50 for fixed-Δ designs. Savings: 5h + $18 per iteration → enables 2× more iterations.
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:378:Round 1-3 patches left inconsistencies between sections on the independent unit and which tests are inferential. **Round 4 rewrites § 5 + § 9 from scratch**. Single source of truth from this point forward; any earlier per-(problem, seed) reference is superseded by per-problem.
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:387:`(problem, seed)` is NOT used as an independent unit anywhere in this PREREG. Any prior reference is superseded.
...
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:449:                 (Lean PASS) AND treatment UNSOLVED for the SAME (problem, seed).
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:512:- C2 **hard-10 adaptation × 2 random Boltzmann routing seeds; independent unit = per-problem (seeds = noise-reducing replicates), not (problem, seed)** (round 3 fix per Codex round-2 R3-1 — within-problem seed correlation). Hard-10 sample is drawn from the adaptation set (144 problems) using the same hash-based deterministic procedure as the three-split: `random.Random("hard10_pput_ccl_seed").sample(adaptation_set, 10)`. The 10 problem IDs + their fingerprint are pre-committed in Phase A5. Boltzmann seeds = `[31415, 2718]`. Total per-mode runs: 5 modes × 10 problems × 2 seeds = 100 jsonl rows. McNemar per condition pair uses **n = 10 per-problem paired-binary observations** (paired sign of `mean(VPPUT_seed1, VPPUT_seed2)_full > mean(VPPUT_seed1, VPPUT_seed2)_alt`). See § 9.2 + § 9.5 for power analysis.
...
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:678:| Phase C all H1-H4 fail | "Three constitutional ablations did not significantly affect VPPUT on hard-10 adaptation × N=20 within Phase C statistical power; either ablations were under-effective or VPPUT signal too noisy at this scale" |
...
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:774:| 2026-04-26 (round 2) | **H1**: heldout-49 → heldout-54 grep-and-replace through PREREG + notepad. Bucket-pseudocode comments updated to "nominal X / realized Y". | Codex audit H1 (round 1 REPRO PASS-with-cleanup) |
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:776:| 2026-04-26 (round 3) | **R3-1**: § 9 rewritten with hard-cap k_max = 10 / N_max = 44; Holm thresholds use N_max throughout (Gate C decisions made before k is known become statistically sound); arc aborts if realized k > 10 (no post-hoc relaxation). § 6 C2 + § 9.2 independent unit changed from (problem, seed) to per-problem with seeds as noise-reducing replicates; n = 10 paired-binary observations for hard-10 McNemar. | Codex round-2 audit P0-fam (still-open) |
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:780:| 2026-04-26 (round 4) | **R4-1**: § 5 + § 9 CLEAN REWRITE — patch-stacking from rounds 1-3 had left inconsistencies (some sections still said `(problem, seed)` n=20, others said per-problem n=10). Round 4 single source of truth: per-problem unit everywhere (n=10 Phase C, n=54 Phase E). Power tables in § 9.4 + § 9.5 corrected (Phase C requires 10/10 paired wins; Phase E ≥ 39/54). | Codex round-3 audit P0-fam (still-open) — internal inconsistency caught |
...
handover/preregistration/PREREG_PPUT_CCL_2026-04-26.md:783:| 2026-04-26 (round 4) | **R4 internal-consistency sweep**: changelog rounds 1-3 all referenced family `4+4k`; round 4 changes to `4+3k` are the canonical source. Earlier `(problem, seed)` references in round 1-2 changelog rows are historical context, not current spec. | self-audit |

codex
**Round-4 Audit**

P0-fam: **closed**. §5.1 makes the independent unit per-problem for both Phase C (`n=10`) and Phase E (`n=54`), with seeds only as replicates. The only remaining `(problem, seed)` uses I see in active spec are explanatory or in §5.5 `p_0` calibration, where same `(problem, seed)` control/treatment pairing is appropriate. §5.6 and §9.1 consistently set `4 + 3k`, `k_max = 10`, `N_max = 34`.

Power: **substantively correct**. For Phase C, one-sided exact binomial at `α = 0.05/34 ≈ 0.00147` requires `10/10` wins (`p=0.000977`). For Phase E, `39/54` is the first passing count at the smallest threshold. Minor note: §9.5’s listed p-values are approximate, not exact, but the cutoff is correct.

P0-gate-h: **closed**. §5.4 cleanly removes `j-RR` from the inferential family and makes it a descriptive point-check guardrail. That fixes the prior mathematical impossibility. This is statistically defensible because `j-RR` is being used as a safety constraint, not as a discovery claim. §1.6, §1.7, §6 E3, §7 H, and §10.1 are synced on “three inferential tests + j-RR point check + rollback.”

P0-leak: **closed for this operational sealing model**. §2.3 L2-L5 now blocks the concrete recomputation path I objected to: seed-bearing prompt context, seed concatenation in tool args, common hash APIs/binaries, SHA-256 manual constants, and MiniF2F/Test enumeration. Residuals still exist in the theoretical sense, such as exotic hardware hash paths, semantic seed paraphrases in an unlisted file, or existence-probe enumeration via `stat/access`; but those are implementation-hardening notes, not prereg blockers, because the document honestly frames sealing as operational rather than cryptographic.

Independent consistency pass:
- I do not see active-spec stale “all four primary inferential tests” language outside historical changelog context.
- One non-blocking stale wording remains in §10.1: “hard-10 adaptation × N=20” should become “hard-10 adaptation, 10 per-problem units, 20 seed-level runs.”
- §6 C2, §6 E3, §1.6, §1.7, and §7 H are aligned with round 4.

**VERDICT**: PASS

Conviction: high. The round-4 rewrite closes my round-3 blockers without introducing a new P0.

Four revision rounds plus a clean rewrite does show the protocol was under-specified earlier, but here it looks like the natural shape of hard preregistration work: the thesis stayed stable while the statistical and sealing machinery was made auditable.
tokens used
111,541
**Round-4 Audit**
