#!/usr/bin/env python3
"""Gemini re-audit for Phase 8 R1-R8 (VETO/CHALLENGE addressment only)."""
...
audit_dir = ROOT / "handover/ai-direct/EXT_AUDIT_PHASE_8_R1R8"
brief = (audit_dir / "brief.md").read_text()
diff = (audit_dir / "amendment.diff").read_text()

# Inline prior audit reports + key current-state files (so auditor doesn't
# need to trust the diff alone).
context_paths = [
    ...
    ".claude/worktrees/phase-8a-snapshot/src/sdk/prompt.rs",
    ".claude/worktrees/phase-8a-snapshot/tests/oracle_receipt_bus.rs",
    ".claude/worktrees/phase-8a-snapshot/tests/q_halt_state.rs",
    ".claude/worktrees/phase-8a-snapshot/tests/reputation.rs",
]
ctx_parts = ["\n\n---\n\n# 附件\n"]
...
full_prompt = (brief
    + "\n\n---\n\n# Amendment diff (commit 2502dc9, +654 -143)\n\n```diff\n"
    + diff + "\n```\n"
    + "".join(ctx_parts))
...
out = ROOT / "handover/audits/EXT_GEMINI_PHASE_8_R1R8_2026-04-22.md"
header = (f"# Gemini Phase 8 R1-R8 Re-Audit\n"
          f"**Date**: 2026-04-22\n"
          f"**Elapsed**: {elapsed:.1f}s\n"
          f"**Prompt size**: {len(full_prompt):,} chars\n"
          f"**Target commit**: 2502dc9 (R1-R8 addressment)\n\n---\n\n")
