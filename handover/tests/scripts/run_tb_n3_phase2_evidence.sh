#!/usr/bin/env bash
# TB-N3 Phase 2 multi-agent (n=5) market-enabled batch runner.
#
# Cloned from `run_tbc0_multi_agent_evidence.sh` (2026-05-06 baseline) with
# TB-N3 env vars set:
#   - TURINGOS_TB_N3_AUTO_MARKET=1     (architect ruling 2026-05-11 amendments 3-6 + Q1+Q2+Q6)
#   - TURINGOS_DEFAULT_POOL_SEED_MICRO=100000  (Q1)
#   - TURINGOS_TB_N3_MARKET_CONTEXT_K=10       (Q7)
#
# Same 9 problems as the TBC0 baseline (architect Q8: "SAME 9 first" for
# direct delta comparison vs Phase 1 evidence).
#
# Per-problem outputs at OUT_DIR/<idx>_<name>:
#   runtime_repo/, cas/, evaluator.{stdout,stderr}, verdict.json,
#   chain_invariant.json, extracted_pput.json,
#   tb_n3_run_report.txt (NEW; audit_dashboard --run-report rendered)
#
# Aggregate outputs at OUT_DIR/:
#   TB_N3_BATCH_SUMMARY.json (NEW; aggregates SG-N3.7..N3.10)
#   TBC0_RUN_MANIFEST.json   (preserved name; consumers index by run dir)
#
# Original TBC0 header preserved below.
# ────────────────────────────────────────────────────────────────────────────
...
DEFAULT_OUT_DIR="$PROJECT_ROOT/handover/evidence/tb_n3_phase2_$(date -u +%Y-%m-%dT%H-%M-%SZ)"
EVALUATOR_BIN="$PROJECT_ROOT/target/release/evaluator"
AUDIT_TAPE_BIN="$PROJECT_ROOT/target/release/audit_tape"
AUDIT_DASHBOARD_BIN="$PROJECT_ROOT/target/release/audit_dashboard"
...
for ((i=0; i<N; i++)); do
  ...
  TURINGOS_CHAINTAPE_PATH="$RUN_DIR/runtime_repo" \
    ...
    TURINGOS_TB_N3_AUTO_MARKET="${TURINGOS_TB_N3_AUTO_MARKET:-1}" \
    TURINGOS_DEFAULT_POOL_SEED_MICRO="${TURINGOS_DEFAULT_POOL_SEED_MICRO:-100000}" \
    TURINGOS_TB_N3_MARKET_CONTEXT_K="${TURINGOS_TB_N3_MARKET_CONTEXT_K:-10}" \
    TURINGOS_TB_N3_MARKET_BUDGET_AGENT="${TURINGOS_TB_N3_MARKET_BUDGET_AGENT:-MarketMakerBudget}" \
...
')"
  ...
  # TB-N3 A5 — render the per-problem run-report (§A..§G) into
  # tb_n3_run_report.txt. Best-effort: if the binary fails (e.g. CAS
  # store transient open issue), we log to stderr but continue.
  if [ -x "$AUDIT_DASHBOARD_BIN" ]; then
    set +e
    "$AUDIT_DASHBOARD_BIN" \
      --repo "$RUN_DIR/runtime_repo" \
      --cas "$RUN_DIR/cas" \
      --run-report \
      --out "$RUN_DIR/tb_n3_run_report.txt" \
      2> "$RUN_DIR/audit_dashboard.stderr" \
      || echo "[tb-n3] audit_dashboard --run-report exit=$? for $TAG" >&2
    set -e
  fi
...
# ── TB-N3 batch summary aggregator (charter v3 §6 SG-N3.7..N3.10) ─────────
echo
echo "[tb-n3] aggregating TB-N3 ship-gate metrics..."
python3 - "$OUT_DIR" <<'PYEOF' > "$OUT_DIR/TB_N3_BATCH_SUMMARY.json" 2> "$OUT_DIR/TB_N3_BATCH_SUMMARY.stderr"
import json, os, sys, glob, re

out_dir = sys.argv[1]
problem_dirs = sorted([d for d in glob.glob(os.path.join(out_dir, "P*_*"))
                       if os.path.isdir(d)])
...
agg = {
    ...
    "totals": {
        ...
        "market_seed_count": 0,
        "complete_set_mint_count": 0,
        "task_open_count": 0,
        "verify_count": 0,
        "finalize_reward_count": 0,
        "event_resolve_count": 0,
        "solved": 0,
        "wall_time_s": 0,
        "no_trade_traces": 0,
        "submitted_traces": 0,
...
def parse_run_report_count(report_path, key):
    """Parse '  Foo: N' line from §C section, return N as int."""
    if not os.path.exists(report_path):
        return 0
    try:
        with open(report_path) as f:
            for line in f:
                m = re.match(r'\s+' + re.escape(key) + r':\s+(\d+)', line)
                if m:
...
def parse_run_report_str(report_path, key):
    """Parse '  Foo: VALUE...' line, return VALUE as raw substring."""
    if not os.path.exists(report_path):
        return None
    try:
        with open(report_path) as f:
...
for pd in problem_dirs:
    tag = os.path.basename(pd)
    rep = os.path.join(pd, "tb_n3_run_report.txt")
    extracted = safe_load(os.path.join(pd, "extracted_pput.json"))
    solved = bool(extracted.get("solved", False))
    work_n = parse_run_report_count(rep, "Work")
    cpmm_pool_n = parse_run_report_count(rep, "CpmmPool")
    router_n = parse_run_report_count(rep, "BuyWithCoinRouter")
    market_seed_n = parse_run_report_count(rep, "MarketSeed")
    csm_n = parse_run_report_count(rep, "CompleteSetMint")
    task_open_n = parse_run_report_count(rep, "TaskOpen")
    verify_n = parse_run_report_count(rep, "Verify")
    finalize_n = parse_run_report_count(rep, "FinalizeReward")
    event_resolve_n = parse_run_report_count(rep, "EventResolve")
    submitted_traces = 0
    no_trade_traces = 0
    pools_skipped = parse_run_report_count(rep, "pools_skipped_budget")
    if os.path.exists(rep):
        try:
            with open(rep) as f:
                for line in f:
                    m = re.match(r'\s+outcome\[submitted\]\s+=\s+(\d+)', line)
                    if m: submitted_traces = int(m.group(1))
                    m = re.match(r'\s+outcome\[no_trade\]\s+=\s+(\d+)', line)
                    if m: no_trade_traces = int(m.group(1))
        except Exception:
            pass
    duration_s = int(extracted.get("duration_s", 0))

    row = {
        "tag": tag,
        "solved": solved,
        "work_accepted": work_n,
        "cpmm_pool_count": cpmm_pool_n,
        "buy_with_coin_router_count": router_n,
        "market_seed_count": market_seed_n,
        "complete_set_mint_count": csm_n,
        "task_open_count": task_open_n,
        "verify_count": verify_n,
        "finalize_reward_count": finalize_n,
        "event_resolve_count": event_resolve_n,
        "submitted_traces": submitted_traces,
        "no_trade_traces": no_trade_traces,
        "pools_skipped_budget": pools_skipped,
    }
    agg["per_problem"].append(row)
    agg["totals"]["accepted_work_tx"] += work_n
    agg["totals"]["cpmm_pool_count"] += cpmm_pool_n
    agg["totals"]["buy_with_coin_router_count"] += router_n
    agg["totals"]["market_seed_count"] += market_seed_n
    agg["totals"]["complete_set_mint_count"] += csm_n
    agg["totals"]["task_open_count"] += task_open_n
    agg["totals"]["verify_count"] += verify_n
    agg["totals"]["finalize_reward_count"] += finalize_n
    agg["totals"]["event_resolve_count"] += event_resolve_n
    if solved:
        agg["totals"]["solved"] += 1
    agg["totals"]["wall_time_s"] += duration_s
    agg["totals"]["submitted_traces"] += submitted_traces
    agg["totals"]["no_trade_traces"] += no_trade_traces
    agg["totals"]["pools_skipped_budget"] += pools_skipped

# Ship-gate evaluation (charter v3 §6 SG-N3.7..N3.10)
acc_w = agg["totals"]["accepted_work_tx"]
pool_c = agg["totals"]["cpmm_pool_count"]
router_c = agg["totals"]["buy_with_coin_router_count"]
solved_c = agg["totals"]["solved"]
wall = agg["totals"]["wall_time_s"]

agg["ship_gates"] = {
    "SG-N3.7_cpmm_pool_ge_accepted_work_tx": {
        "expected": f">= {acc_w}",
        "actual": pool_c,
        "pass": pool_c >= acc_w if acc_w > 0 else True,
    },
    "SG-N3.8_buy_with_coin_router_ge_1": {
        "expected": ">= 1",
        "actual": router_c,
        "pass": router_c >= 1,
    },
    "SG-N3.9_solved_ge_4_of_9": {
        "expected": ">= 4 / 9",
        "actual": f"{solved_c} / {len(problem_dirs)}",
        "pass": solved_c >= 4,
    },
    "SG-N3.10_wall_time_le_14400_s": {
        "expected": "<= 14400 s",
        "actual": wall,
        "pass": wall <= 14400,
    },
...
python3 - "$OUT_DIR/TB_N3_BATCH_SUMMARY.json" <<'PYEOF'
import json, sys
try:
    with open(sys.argv[1]) as f: a = json.load(f)
except Exception as e:
    print(f"[tb-n3] BATCH_SUMMARY parse error: {e}", file=sys.stderr); sys.exit(0)
print()
print("=" * 60)
print("TB-N3 PHASE 2 BATCH SUMMARY")
print("=" * 60)
print(f"problems: {a['n_problems']}")
print(f"solved: {a['totals']['solved']} / {a['n_problems']}")
print(f"accepted_work_tx: {a['totals']['accepted_work_tx']}")
print(f"cpmm_pool_count: {a['totals']['cpmm_pool_count']}")
print(f"buy_with_coin_router: {a['totals']['buy_with_coin_router_count']}")
print(f"market_seed: {a['totals']['market_seed_count']}")
print(f"submitted_traces: {a['totals']['submitted_traces']}")
print(f"no_trade_traces: {a['totals']['no_trade_traces']}")
print(f"wall_time: {a['totals']['wall_time_s']}s")
print()
print("Ship gates:")
for sg, d in a.get("ship_gates", {}).items():
    status = "PASS" if d["pass"] else "FAIL"
    print(f"  {status} {sg}: actual={d['actual']} (expected {d['expected']})")
print("=" * 60)
PYEOF

echo
echo "[tb-n3] DONE → $OUT_DIR"
echo "[tb-n3] TB-N3 summary: $OUT_DIR/TB_N3_BATCH_SUMMARY.json"
echo "[tb-n3] TBC0 summary:  $OUT_DIR/TBC0_BATCH_SUMMARY.json"
echo "[tb-n3] FC aggregate:  $OUT_DIR/fc_witness_aggregate.json"

if [ "$EVAL_FAIL_COUNT" -gt 0 ]; then
  echo "[tb-n3] WARN: $EVAL_FAIL_COUNT evaluator failures (non-timeout)" >&2
fi
exit 0
