...
#[test]
fn loop_once_records_claim_and_worker_report_as_first_class_devtape_events() {
    let dir = temp_path("claim-report");
    fs::create_dir_all(&dir).expect("temp dir should be created");
    let store = dir.join("events.jsonl");
    let board = dir.join("TASK_BOARD.json");
    let prs = dir.join("prs.json");
    let atom = "V5-LOOP-CLAIM-REPORT";
    seed_task(&store, atom, "open", None, None);
    write_json(
        &prs,
        &json!([
            {
                "number": 43,
                "title": "[CLAIM][V5-LOOP-CLAIM-REPORT][Class1] Worker claim",
                "state": "OPEN",
                "isDraft": false,
                "createdAt": "2026-05-20T08:35:56Z",
                "mergedAt": null,
                "url": "https://github.com/gretjia/turingosv5/pull/43",
                "body": "ClaimRecord\n- atom_id: V5-LOOP-CLAIM-REPORT\n\nWorkerReport\n- worker_halt_confirmation: \"[WORKER_HALT]\"\n",
    ...
    assert!(
        records.iter().any(|record| {
            record.envelope["event_type"] == "TaskClaimed"
                && record.payload["atom_id"] == atom
                && record.payload["pr_number"] == 43
        }),
        "loop once must materialize GitHub claim as TaskClaimed"
    );
    assert!(
        records.iter().any(|record| {
            record.envelope["event_type"] == "WorkerReportSubmitted"
                && record.payload["atom_id"] == atom
                && record.payload["pr_number"] == 43
        }),
        "loop once must materialize WorkerReport as WorkerReportSubmitted"
    );
    ...
    assert_eq!(row["pr_number"], 43);
}

#[test]
fn loop_once_records_deterministic_gate_evidence_for_merge_check() {
    let dir = temp_path("deterministic-gates");
    fs::create_dir_all(&dir).expect("temp dir should be created");
    let store = dir.join("events.jsonl");
    let board = dir.join("TASK_BOARD.json");
    let prs = dir.join("prs.json");
    let atom = "V5-LOOP-GATE-EVIDENCE";
    ...
    let tip = append(
        ...
        json!({
            "atom_id": atom,
            "pr_number": 44,
            "claim_pr_url": "https://github.com/gretjia/turingosv5/pull/44",
            "createdAt": "2026-05-20T08:35:56Z"
        }),
    );
    append(
    ...
    write_json(
        ...
        &json!([
            {
                "number": 44,
                "title": "[CLAIM][V5-LOOP-GATE-EVIDENCE][Class1] Worker claim",
                "state": "OPEN",
                "isDraft": false,
                "createdAt": "2026-05-20T08:35:56Z",
                "mergedAt": null,
                "url": "https://github.com/gretjia/turingosv5/pull/44",
                "body": "ClaimRecord\n- atom_id: V5-LOOP-GATE-EVIDENCE\n\nWorkerReport\n- worker_halt_confirmation: \"[WORKER_HALT]\"\n",
                "mergeStateStatus": "CLEAN",
    ...
    assert!(records.iter().any(|record| {
        record.envelope["event_type"] == "AuditVerdictSubmitted"
            && record.payload["pr_number"] == 44
            && record.payload["verdict"] == "PASS"
    }));
    assert!(records.iter().any(|record| {
        record.envelope["event_type"] == "VetoVerdictSubmitted"
            && record.payload["pr_number"] == 44
            && record.payload["verdict"] == "PASS"
    }));
    assert!(records.iter().any(|record| {
        record.envelope["event_type"] == "MergeDecisionRecorded"
            && record.payload["pr_number"] == 44
            && record.payload["decision"] == "PROCEED"
    }));

    let merge_check = Command::new(bin())
        .args([
            "merge",
            "check",
            "--store",
        ...
        .output()
        .expect("merge check should run");
    assert!(
        merge_check.status.success(),
        "stderr: {} stdout: {}",
        String::from_utf8_lossy(&merge_check.stderr),
        String::from_utf8_lossy(&merge_check.stdout)
    );

    let second_output = Command::new(bin())
        .args([
            "loop",
            "once",
            "--store",
    ...
    let records_after_second_loop = read_records(&store).expect("records should read");
    let gate_event_count = records_after_second_loop
        .iter()
        .filter(|record| {
            record.payload["pr_number"] == 44
                && matches!(
    ...
    assert_eq!(
        gate_event_count, 4,
        "polling loop must not duplicate unchanged followup evidence"
    );
}
